<template>
    <div class="flex-1 flex items-center gap-2 pt-1 text-slate-500">
        <div v-for="{ id, icon } in tabs" class="border-b-4 py-2" :class="{'border-white': id !== active, 'border-indigo-500 text-indigo-500': id === active}" @click="changeView(id)">
            <button class="flex gap-1 items-center px-2 py-1 rounded-md cursor-pointer hover:bg-slate-100">
...
<script setup lang="ts">
    ...
    const tabs = [{id: "list", icon: "table_rows"}, {id: "grid", icon: "calendar_view_month"}, {id: "board", icon: "view_kanban"}, {id: "calendar", icon: "calendar_month"}, {id: "timeline", icon: "view_timeline"}];
